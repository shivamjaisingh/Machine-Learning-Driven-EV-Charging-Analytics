...
df_open_transactions = df_open_transactions[
    ['TotalEnergy', 'Season', 'ConnectedTime', 'Day of the Week', 'MaxPower', 'TotalEnergy']]

df_open_transactions = pd.get_dummies(df_open_transactions)

label = np.array(df_open_transactions['TotalEnergy'])

df_open_transactions = df_open_transactions.drop('TotalEnergy', axis=1)
...
xgb_m = xgb.XGBRegressor(objective="reg:linear", random_state=42)
...
print('Result for Energy Consumption')
