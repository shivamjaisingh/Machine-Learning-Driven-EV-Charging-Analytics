...
df_open_transactions = df_open_transactions[
    ['TotalEnergy', 'Season', 'ConnectedTime', 'Day of the Week', 'MaxPower', 'IdleTimeRatio']]

df_open_transactions = pd.get_dummies(df_open_transactions)

label = np.array(df_open_transactions['IdleTimeRatio'])

df_open_transactions = df_open_transactions.drop('IdleTimeRatio', axis=1)
